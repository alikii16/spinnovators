...
public class YearComparisonWebDisplay {
  ...
  public String generateComparisonContent(
    ...
    if (totalBudgets != null) {
      ...
      if (baseFromMap != null) {
        baseTotal = baseFromMap;
      }

      if (compareFromMap != null) {
        compareTotal = compareFromMap;
      }
    }
    StringBuilder html = new StringBuilder();
    html.append("<h2 class='section-title'>Σύγκριση Προϋπολογισμού: ")
        .append(baseYearStr)
        .append(" vs ")
        .append(compareYearStr)
        .append("</h2>");
  ...
  private String buildTotalChange(
    ...
    return "<div style='margin-top: 32px; padding-top: 24px; border-top: 2px solid #c8e6c9;'>"
      ...
      + "margin-bottom: 20px; text-align: center;'>Συνολική Αλλαγή Προϋπολογισμού</h3>"
      + "<div style='padding: 16px; background: #f1f8e9; border: 2px solid #0d4f1c; "
      + "border-radius: 10px; text-align: center;'>"
      + String.format(
    HELLENIC_LOCALE,
    "<p style='color: %s; font-weight: 700; font-size: 18px;'>Διαφορά: %+,.2f €</p>",
    diffColor,
    totalDiff)
  ...
  private String buildEsgComparison(
    ...
    html.append(
        "<h3 style='font-size: 20px; font-weight: 700; color: #0d4f1c; "
        + "margin-bottom: 20px; text-align: center;'>Σύγκριση ESG Score</h3>");
    html.append(
        "<div style='display: flex; gap: 20px; margin-bottom: 32px; "
        + "flex-wrap: wrap; justify-content: center;'>");
    html.append(
        buildEsgScoreBox(
        "ESG Score " + baseYearStr,
        baseReport.getOverallScore(),
        "#e8f5e9",
        "#0d4f1c"));
    html.append(
        buildEsgScoreBox(
        "ESG Score " + compareYearStr,
        compareReport.getOverallScore(),
        "#e8f5e9",
        "#0d4f1c"));
    ...
    html.append(
        ...
        + "border-top: 2px solid #c8e6c9;'>");
    html.append(
        "<h4 style='font-size: 18px; font-weight: 700; color: #0d4f1c; "
        + "margin-bottom: 16px; text-align: center;'>Λεπτομέρειες Αλλαγών</h4>");
    html.append("<div style='display: flex; flex-direction: column; gap: 12px;'>");
    html.append(
        buildEsgCategoryRow(
        "Environmental (E)",
        baseReport.getEnvironmentalScore(),
        compareReport.getEnvironmentalScore()));
    html.append(
        buildEsgCategoryRow(
        "Social (S)",
        baseReport.getSocialScore(),
        compareReport.getSocialScore()));
    html.append(
        buildEsgCategoryRow(
        "Governance (G)",
        baseReport.getGovernanceScore(),
        compareReport.getGovernanceScore()));
    html.append("</div>");
    html.append(
  ...
  private String buildEsgScoreBox(
      ...
      String message) {
    StringBuilder html = new StringBuilder();
    html.append("<div style='flex: 1; min-width: 180px; background: ")
    .append(bgColor)
    ...
    if (message == null) {
      html.append(
          String.format(
          HELLENIC_LOCALE,
          "<div style='font-size: 32px; font-weight: 700; color: #0d4f1c; "
      ...
      html.append(
          ...
          HELLENIC_LOCALE,
          "<div style='font-size: 32px; font-weight: 700; color: #0d4f1c; "
          + "margin-bottom: 4px;'>%.2f points</div>",
          value));
      html.append(
    "<div style='font-size: 12px; color: #1b5e20; font-weight: 600; margin-top: 8px;'>")
    .append(message)
          .append("</div>");
    }
    html.append("</div>");
  ...
  private String buildEsgCategoryRow(String label, double baseScore, double compareScore) {
    ...
    String bg =
        diffInfo[0].equals("positive")
        ? "#e8f5e9"
        : (diffInfo[0].equals("negative")
            ? "#ffebee"
            : "#f5f5f5");
    String color =
        diffInfo[0].equals("positive")
    ...
    return "<div style='display: flex; justify-content: space-between; align-items: center; "
        ...
        + String.format(
        HELLENIC_LOCALE,
        "<span style='font-size: 15px; color: #2e7d32; font-weight: 600;'>%.1f%%</span>",
    baseScore)
        ...
        "<span style='font-size: 15px; color: #2e7d32; font-weight: 600;'>%.1f%%</span>",
        compareScore)
        + "<span style='padding: 4px 8px; border-radius: 4px; font-weight: 700; background: "
        + bg
        + "; color: "
        + color
        + ";'>"
