...
public class EnvBudgetLoader {
    ...
    /** JSON file to load from classpath. */
     ...
     * @throws EnvDataLoadException if file reading or parsing fails.
     */
    public EnvBudgetData loadBudget() throws EnvDataLoadException {
        InputStream is = getClass().getResourceAsStream(JSON_FILE_NAME);
        if (is == null) {
            throw new EnvDataLoadException(JSON_FILE_NAME
                + " Error: JSON file not found. Ensure it is in "
                + "src/main/resources.");
        }

        try (InputStream nonNullIs = is;
            ...
            if (rootMap == null || !rootMap.containsKey("data_by_year")) {
                throw new EnvDataLoadException(
                    "Error: JSON structure invalid (missing 'data_by_year').");
            }

            Map<String, Double> totalBudget = transformTotalBudget(
                (Map<String, Object>) rootMap.get(
    ...
    /**
     * Transforms raw total budget map into typed map.
     *
     * @param budgetMap raw JSON map.
     * @return map of budget per year.
     */
    private Map<String, Double> transformTotalBudget(
        final Map<String, Object> budgetMap) {
    ...
    /**
     * Transforms raw JSON year map into EnvYear objects.
     *
     * @param yearsMap raw map from JSON.
     * @return structured map of EnvYear objects.
     */
    private Map<String, EnvYear> transformYears(
        final Map<String, Object> yearsMap) {
        Map<String, EnvYear> envYears = new HashMap<>();
        if (yearsMap != null) {
            for (Map.Entry<String, Object> entry : yearsMap.entrySet()) {
                String year = entry.getKey();
                Map<String, Object> sectorsMap =
    ...
    /**
     * Constructs EnvUnit objects from raw map.
     *
     * @param unitsMap raw map from JSON.
     * @return list of EnvUnit objects.
     */
    ...
    /**
     * Constructs EnvEntry objects from raw map.
     *
     * @param entriesMap raw map from JSON.
     * @return list of EnvEntry objects.
     */
    private List<EnvEntry> transformEntries(
        final Map<String, Object> entriesMap) {
